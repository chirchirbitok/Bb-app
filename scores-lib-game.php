...
class Game {
  ...
  // PARAM : DB_HOST, DB_CHARSET, DBNAME, DB_USER, DB_PASSWORD

    // ATTEMPT CONNECT
    try {
      $str = "mysql:host=" . HOSTNAME . ";charset=" . DB_CHARSET;
      if (defined('DBNAME')) { $str .= ";dbname=" . DBNAME; }
      $this->pdo = new PDO(
        $str, HOSTUSER, HOSTPASSWORD, [
  ...
  // getAll() : get all games
    ...
    $sql = "SELECT game_score.game_id, game_start, game_home,  MAX(score_home) as score_home, game_away, MAX(score_away) as score_away FROM `game_score` INNER JOIN `game` ON `game_score`.`game_id`=`game`.`game_id` GROUP BY game_home";
    $game = $this->fetch($sql);
    return count($game)==0 ? false : $game ; 
  }
